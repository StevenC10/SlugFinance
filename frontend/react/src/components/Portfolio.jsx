...
const fetchPortfolio = (setPortfolio) => {
  ...
  const result = fetch(
    "http://127.0.0.1:5000/v0/getFromPortfolio?email=" + item,
    {
      method: "GET",
      headers: new Headers({
  ...
  result.then((tickers) => {
    console.log(tickers);
    if (tickers !== "No email found") {
      // If the user has tickers in portfolio
      for (const ticker of tickers) {
        // For each ticker in user's portfolio
        const getInfo = { ticker: ticker };
        const adding = fetch("http://127.0.0.1:5000/v0/add", {
          // Adding the ticker to the table
          method: "POST",
        ...
        adding.then((add) => {
          // Nested fetchc all to get the ticker from the table
          fetch("http://127.0.0.1:5000/v0/ticker?id=" + ticker, {
            ...
            .then((json) => {
              ...
              if (info.length === tickers.length) {
                // Only set state when all fetches are done
        ...
        >
          <div className="text-sm">
            <div className="font-medium text-gray-700">
              {portfolio[i][0][0][0].toUpperCase()}
            </div>
            <div className="text-gray-400">{portfolio[i][0][0][1]}</div>
          </div>
        </th>
      );
      column.push(
        <td onClick={redirect} className="px-6 py-4 font-medium">
          {portfolio[i][0][0][3]}
        </td>
      );
      column.push(
        ...
        <td className="px-6 py-4">
          <div className="flex justify-end gap-4">
            <svg
              ...
              aria-label="remove"
              onClick={() =>
                removeFromPortfolio(portfolio[i][0][0][0].toUpperCase())
              }
            >
              <path
                strokeLinecap="round"
