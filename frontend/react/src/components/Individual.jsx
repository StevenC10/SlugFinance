...
const Individual = () => {
  ...
  if (
    ...
    tickerInfo.push(
      <p className="text-2xl font-sans font-semibold pl-2 pt-2 inline-block uppercase">
        {priceInfo[0][0][0]}
      </p>
    );
    tickerInfo.push(
      <p className="text-2xl font-sans font-bold pl-2 inline-block">
        {priceInfo[0][0][2]}
      </p>
    );
    if (priceInfo[0][0][3][0] === "+") {
      // Green text if price change is positive, red if negative
      tickerInfo.push(
        <p className="text-2xl font-sans font-semibold pl-2 inline-block text-green-600">
          {priceInfo[0][0][3]}
        </p>
      );
      tickerInfo.push(
      ...
      tickerInfo.push(
        <p className="text-2xl font-sans font-semibold pl-2 inline-block text-red-600">
          {priceInfo[0][0][3]}
        </p>
      );
      tickerInfo.push(
    ...
    tickerInfo.push(
      <button
        type="button"
        onClick={() => addToPortfolio(priceInfo[0][0][0].toUpperCase())}
        className="w-1/6 py-1 mt-2 font-semibold border rounded float-right bg-blue-500"
      >
        Add to myPortfolio
      </button>
    );
    ...
    for (const key in info[0][0][1][0]) {
      // For each information portion
      counter++; // Maintaining counter to display different div style based on which number row
      if (counter % 4 === 1) {
        row.push(
          <div className="flex flex-col pb-3">
            <dt key={key} className="mb-1 text-gray-400 md:text-lg">
              {key}
            </dt>
            <dd key={info[0][0][1][0][key]} className="text-lg font-semibold">
              {info[0][0][1][0][key]}
            </dd>
          </div>
        );
        ...
        viewInformation.push(
          <div key={counter} className="w-1/3">
            <dl className="w-5/6 text-gray-900 divide-y divide-gray-200">
              {row}
            </dl>
          </div>
        );
    ...
    <div className="h-screen">
      ...
      <header className="top-0 sm:px-12 mx-auto flex items-center p-4 bg-blue-900">
        <div className="container flex justify-between h-10 mx-auto">
          <a
            ...
            className="flex items-center p-2 mx-0"
          >
          ...
          <div className="mb-6 items-center justify-start">
            <form onSubmit={handleSubmit}>
              <input
                ...
                className="w-full px-4 py-2 placeholder-gray-500 border border-gray-200 rounded-md focus:outline-none bg-gray-20 border-gray-600"
              />
            </form>
          </div>
          <div className="items-center space-x-2 flex-shrink-0 hidden lg:flex">
            <li className="flex">
              {localStorage.getItem("user") !== null ? (
                <button
                  type="button"
                  className="px-8 py-3 font-semibold rounded-full text-gray-800 bg-gray-300 hover:bg-gray-400 font-semibold active:bg-gray-500"
                  onClick={toPortfolio}
                >
                  myPortfolio
                </button>
                ...
                <button
                  type="button"
                  className="px-8 py-3 font-semibold rounded-full bg-gray-500 text-gray-800"
                  onClick={toPortfolio}
                >
                  myPortfolio
                </button>
            ...
            {localStorage.getItem("user") !== null ? (
              <button
                className="self-center px-8 py-3 rounded text-gray-200 bg-red-600 hover:bg-red-700 font-semibold active:bg-red-800"
                onClick={toLogout}
              >
                Log out
              </button>
            ...
            {localStorage.getItem("user") === null ? (
              <button
                className="self-center px-8 py-3 rounded text-gray-200 bg-blue-600 hover:bg-blue-700 font-semibold active:bg-blue-800"
                onClick={toSignup}
              >
                Sign up
              </button>
            ...
            {localStorage.getItem("user") === null ? (
              <button
                className="self-center px-8 py-3 rounded text-gray-200 bg-gray-600 hover:bg-gray-700 font-semibold active:bg-gray-800"
                onClick={toLogin}
              >
                Log in
              </button>
      ...
      <div className="flex h-3/5">
        <div className="w-full h-full border-2">
          ...
          <button
            ...
            className="w-1/12 mr-2 py-1 mt-2 font-semibold border rounded float-right"
          >
            Line
          </button>
          <button
          ...
          <button
            ...
            className="w-1/4 py-3 font-semibold border rounded"
          >
            1 Week
          </button>
          <button
      ...
      <div className="flex h-2/5">
        <div className="w-3/4 border-r h-full w-full">
          ...
          <div className="flex px-2">{viewInformation}</div>
        </div>
        <div className="w-1/4">
